...
function foldersInSchemaFolder() {
    var files = (0, fs_1.readdirSync)(path.join(__dirname), { withFileTypes: true });
    var folders = files.filter(function (file) { return file.isDirectory(); }).map(function (folder) { return folder.name; });
    return folders;
}
function returnTypeDefs() {
    var folders = foldersInSchemaFolder();
    var typesDefs = folders.map(function (type) {
        var schemaDir = path.join(__dirname, type);
        var schemaFiles = (0, fs_1.readdirSync)(schemaDir).filter(function (file) { return file.endsWith('.graphql'); });
        var schema = schemaFiles.map(function (file) {
            var filePath = path.join(schemaDir, file);
...
function writeTypeDefs(filePath, typeDefs) {
    ...
    (0, fs_1.writeFile)(filePath, all, function (err) {
        if (err) {
...
function getTypeDeftsInProduction() {
    ...
    return __awaiter(this, void 0, void 0, function () {
        var stream, result, _d, stream_1, stream_1_1, chunk, e_1_1;
        return __generator(this, function (_e) {
            switch (_e.label) {
                case 0:
                    stream = (0, fs_1.createReadStream)(path.join(__dirname) + '/typedefs.txt', { encoding: 'utf8' });
                    result = '';
                    _e.label = 1;
                case 1:
                ...
                case 7:
                    ...
                    if (!(!_d && !_a && (_b = stream_1.return))) return [3 /*break*/, 9];
